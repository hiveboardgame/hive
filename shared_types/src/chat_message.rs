...
fn truncate_chat_message(text: &mut String) {
    // Count in characters, not bytes: the input enforces maxlength in
    // characters and the DB CHECK is `char_length(body) <= 1000`. A byte-based
    // cap would silently shorten multibyte (e.g. CJK) messages to a fraction of
