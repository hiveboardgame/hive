...
On success the server binds the socket to your bot account and replies
`{"Ok": {"Authenticated": {...your account...}}}`, followed by a `LobbySnapshot` with your
ongoing games and challenges. On failure it replies `{"Ok": {"Error": "Auth failed"}}` and the
socket stays anonymous. You can send another `Auth` frame to retry.

`Authenticated` is the only frame that means your `Auth` landed, and it is how you learn your own
`uid` — nothing else tells you, and you need it to tell which colour you are playing. Wait for it
rather than for the next message: the socket joins the lobby before it authenticates, so
broadcasts, pings, and the anonymous connection's own snapshot can all arrive in between.
...
| message | when |
|---|---|
| `{"Authenticated": {...}}` | your `Auth` succeeded. Your own account, including the `uid` you play as |
| `{"LobbySnapshot": {...}}` | on connect, after `Auth`, and on `Resync`. Carries your ongoing games and challenges |
...
updates for. The `LobbySnapshot` sent after `Auth` gives you enough to rebuild your state.
...
then guarantees your bot matches the server, and you skip this document entirely. `hive-hydra` in
this repo is the example to copy.
