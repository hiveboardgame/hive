...
fn threefold_detection_matches_recorded_conclusions() {
    ...
    for row in games[1..].iter() {
        ...
        let Ok(history) = History::new_from_str(&row[3], GameType::MLP) else {
            continue;
        };
...
fn detected_repetitions_are_real() {
    ...
    for row in games[1..].iter().filter(|r| wanted.contains(&r[0])) {
        seen += 1;
        let history = History::new_from_str(&row[3], GameType::MLP).expect("history");
...
fn board_results_match_the_recorded_winner() {
    ...
    for row in games[1..].iter() {
        ...
        // A record that ends before the win cannot be replayed into one. Only the position can
        // say so, and it has to say it independently: `game_status` is derived from
        // `board.game_result()`, so asking that would agree with itself and excuse every miss.
        if ours == "InProgress"
            && crate::fixture_check::surrounded_queens(&state.board) == GameResult::Unknown
        {
    ...
    assert!(checked > 0, "no board-concluded games were checked");
    // Not an id allowlist: a short record means the games export was taken before the game
    // finished, so the exact set belongs to one pair of snapshots and not to the engine. What
    // has to hold is that they stay a rounding error - if they are not, the two CSVs are far
    // enough apart that the agreement above is being measured on the wrong games.
    assert!(
        short_records.len() * 20 < checked,
        "{} of {checked} board-concluded games end before their recorded win: are both CSVs \
