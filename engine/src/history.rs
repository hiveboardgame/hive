...
impl History {
    ...
    /// Parses UHP and replays its moves to validate the history against the game rules.
    pub fn from_uhp_str(string: &str) -> Result<Self, GameError> {
        let mut history = Self::parse_uhp_str_without_replay(string)?;
    ...
    /// Parses UHP syntax without replaying moves or validating them against the game rules.
    pub fn parse_uhp_str_without_replay(string: &str) -> Result<Self, GameError> {
        let mut history = History::new();
...
mod tests {
    ...
    fn parse_uhp_keeps_moves_for_caller_validation() {
        let history = History::parse_uhp_str_without_replay(PARTIAL_GAME).unwrap();
