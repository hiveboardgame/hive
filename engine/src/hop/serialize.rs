...
pub fn from_position(board: &Board, game_type: GameType, to_move: Color) -> String {
    from_position_oriented(board, game_type, to_move, Orientation::default())
}

pub fn from_position_oriented(
    board: &Board,
    game_type: GameType,
    to_move: Color,
    ...
    match serialize_game_type(game_type) {
        Some(prefix) => format!("{prefix},{topology},{player}{orientation}"),
        None => format!("{topology},{player}{orientation}"),
    }
}
...
fn best_topology(board: &Board) -> String {
    ...
    // Brute-forced over every connected shape up to 10 cells, so a covering walk always exists.
    // Debug-only: a release build serializes an uncovered hive as the empty board (see
    // hop-shitlist.md).
    debug_assert!(
        cells.is_empty() || best.is_some(),
        "chain walker failed to cover a non-empty hive"
    );
...
/// `base`, or `ultimate` written by leaving the field out. The partial-expansion spellings exist
/// for legacy games; [`super::parse`] accepts exactly these, so every emitted HOP round-trips.
fn serialize_game_type(game_type: GameType) -> Option<&'static str> {
    match game_type {
        GameType::MLP => None,
        GameType::Base => Some("base"),
        GameType::M => Some("base+Mm"),
        GameType::L => Some("base+Ll"),
        GameType::P => Some("base+Pp"),
        GameType::ML => Some("base+MLml"),
        GameType::LP => Some("base+LPlp"),
        GameType::MP => Some("base+MPmp"),
    }
}
