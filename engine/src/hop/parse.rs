...
struct Walk {
    ...
    pending_branch: bool,
    /// The bug the immediately preceding token placed; `!` may mark that and nothing else, so
    /// every other token clears it.
    markable: Option<(Piece, Position)>,
...
impl Walk {
    ...
    fn mark_last_moved(&mut self) -> Result<(), HopError> {
        if self.marked.is_some() {
            return Err(HopError::MisplacedMark);
        }
        self.marked = Some(self.markable.ok_or(HopError::MisplacedMark)?);
        Ok(())
    }
