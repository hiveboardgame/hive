...
/// Only one piece can have moved last, so a second mark is a contradiction rather than a
/// stronger claim - and `Q!qA!,w` used to resolve it silently in favour of the later bug.
#[test]
fn mark_must_directly_follow_its_bug() {
    assert!(parse("Q!,b").is_ok());
    assert!(parse("Q1=B!,b").is_ok());
    assert_eq!(parse("Q+!A-a,w").unwrap_err(), HopError::MisplacedMark);
    assert_eq!(parse("Q!!,b").unwrap_err(), HopError::MisplacedMark);
    assert_eq!(parse("Q!qA!,w").unwrap_err(), HopError::MisplacedMark);
}
