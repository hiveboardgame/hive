...
pub async fn run_game_stats(
    ...
    let games_to_analyze = if let Some(sample_size) = sample_size {
        if sample_size >= all_games.len() {
            info!("Sample size {} is >= total games {}, analyzing all games", sample_size, all_games.len());
            all_games
        } else {
            info!("Randomly sampling {} games from {} total games", sample_size, all_games.len());
    ...
    let max_turns = games_to_analyze.iter()
        .map(|game| game.turn)
        .max()
...
fn sample_games_randomly(games: &[Game], sample_size: usize) -> Vec<Game> {
    ...
    let mut rng = rng();
    games.choose_multiple(&mut rng, sample_size).cloned().collect()
}