...
use chrono::Utc;
use log::info;
use crate::common::{setup_database, log_operation_start, log_operation_complete};
...
pub async fn list_users(database_url: Option<String>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let users = db_lib::schema::users::table
        .select((db_lib::schema::users::id, db_lib::schema::users::username, db_lib::schema::users::created_at))
    ...
    for (id, username, created_at) in users {
        info!("  {} ({}) - created {}", username, id, created_at.format("%Y-%m-%d %H:%M:%S"));
    }
