...
async fn handle_request(
    ...
    if let ClientRequest::Auth(token) = request {
        // Every accepted repeat costs a disconnect/reconnect, a login query, a spawned state
        // loader and a full snapshot. Worse, loaders identify a socket only by
        // (user_id, socket_id), so reinserting the same pair makes queued loaders from the
        // previous round valid again and they all run.
        if identity.lock().expect("identity mutex poisoned").authed {
            send_direct(
                hub,
                socket,
                ServerMessage::Error("Already authenticated".to_string()),
            )
            .await;
            return;
        }
...
async fn handle_auth(
    ...
    // Queued before on_connect so it cannot lose to the snapshot that on_connect schedules.
    // The client has to have one frame it can trust, and the socket's own queue is the only
    // thing that orders these two.
    send_direct(
        hub,
        socket,
    ...
    // survive the swap. on_connect also schedules the lobby snapshot the bot needs, which is
    // why there is no explicit one here.
...
mod tests {
    ...
    #[test]
    fn auth_ok_matches_the_documented_frame() {
        use crate::common::{ServerMessage, ServerResult};

        let message = ServerResult::Ok(Box::new(ServerMessage::AuthOk {
            username: "bot".to_string(),
        }));

        assert_eq!(
            serde_json::to_string(&message).expect("encodes"),
            r#"{"Ok":{"AuthOk":{"username":"bot"}}}"#
        );
    }

    #[tokio::test]
    async fn a_second_auth_on_an_authenticated_socket_is_refused() {
        use crate::{
        ...
        let identity = Arc::new(Mutex::new(bot));
        let jwt_secret = Arc::new(JwtSecret::new("test-secret".to_string()));

        // Every accepted repeat costs a disconnect/reconnect, a login query, a spawned state
        // loader and a full snapshot, so the socket must not reach handle_auth at all.
        super::handle_request(
            ClientRequest::Auth("another-token".to_string()),
            &hub,
            &socket,
            &data,
            &pool,
        ...
        let frame = rx
            .try_recv()
            .expect("the server answers the duplicate Auth");
        let text = String::from_utf8(frame.to_vec()).expect("a JSON socket gets text");
        assert!(
            text.contains("Already authenticated"),
            "expected a refusal, got {text}"
        );
    }
