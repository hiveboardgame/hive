...
/// A JSON client parses the nonce through a double, so anything wider than 53 bits comes back
/// rounded and never matches the pong — leaving RTT and lag compensation stuck at zero.
fn json_safe_nonce(raw: u64) -> u64 {
    raw & ((1 << 53) - 1)
}
...
impl WsHub {
    ...
    pub fn on_connect(
        ...
        actix_rt::spawn(async move {
            ...
            let Some(socket) = self.snapshot_socket(user_id, socket_id) else {
                return;
            };
            let _guard = InFlightGuard::new(self.data.telemetry.clone());
            self.load_user_state(socket, user, broadcast_online).await;
            drop(permit);
        });
    }
    ...
    pub fn ping_all(&self) {
        ...
        for user_id in user_ids {
            let nonce = json_safe_nonce(rng.random::<u64>());
    ...
    /// The `SocketTx` the connect-time snapshot goes out on. The loader runs long after
    /// `on_connect` returned, so the handle is looked up again rather than captured.
    fn snapshot_socket(&self, user_id: Uuid, socket_id: Uuid) -> Option<SocketTx> {
        let (tx, format) = self.sessions.get(&user_id).and_then(|socks| {
            socks
                .get(&socket_id)
                .map(|handle| (handle.tx.clone(), handle.format))
        })?;
        Some(SocketTx {
            socket_id,
            format,
    ...
    async fn load_user_state(&self, socket: SocketTx, user: SimpleUser, broadcast_online: bool) {
...
mod tests {
    ...
    #[tokio::test]
    async fn the_connect_snapshot_speaks_the_negotiated_format() {
        let hub = make_hub().await;
        let user_id = Uuid::new_v4();
        let socket_id = Uuid::new_v4();
        let _rx = hub.register_socket_as(user_id, socket_id, SocketFormat::Json);

        let socket = hub
            .snapshot_socket(user_id, socket_id)
            .expect("the socket is registered");

        assert_eq!(
            socket.format,
            SocketFormat::Json,
            "the loader runs after on_connect returns and must not re-guess the format"
        );
    }

    #[test]
    fn ping_nonces_fit_a_json_number() {
        // A JS client parses the nonce through a double, so anything above 2^53 comes back
        // rounded and never matches the pong we are waiting for.
        const MAX_SAFE: u64 = (1 << 53) - 1;

        assert!(json_safe_nonce(u64::MAX) <= MAX_SAFE);
        assert!(json_safe_nonce(MAX_SAFE + 1) <= MAX_SAFE);
        assert_eq!(json_safe_nonce(12_345), 12_345);
    }
