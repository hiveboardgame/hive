...
pub(crate) fn navigate_analysis_history(
    ...
    let updated_node_id = action.target_node_id(analysis.store);
...
mod tests {
    use super::*;
    use hive_lib::{Color, GameType, History};

    #[test]
    fn analysis_navigation_syncs_reserve_from_the_selected_node() {
        let owner = Owner::new();
        owner.with(|| {
            ...
            store
                .load_pgn(
                    game_state,
                    include_str!("../../../engine/test_pgns/valid/p_game.pgn"),
                )
                .expect("valid analysis");
            let previous_node = store.selected_node_id_untracked();
            let synced_color = RwSignal::new(None);
            let synced_node = RwSignal::new(None);
            let analysis = AnalysisContext::new(
                store,
                Callback::new(|_: Color| {}),
                Callback::new(|_: ()| {}),
                Callback::new(move |color| {
                    synced_node.set(Some(store.selected_node_id_untracked()));
                    synced_color.set(Some(color));
                }),
            );

            assert!(navigate_analysis_history(
                AnalysisHistoryNavigation::Previous,
                analysis,
                game_state,
            ));

            let selected_color = game_state.state().with_untracked(|state| state.turn_color);
            let selected_node = store.selected_node_id_untracked();
            assert_ne!(selected_node, previous_node);
            assert_eq!(synced_node.get_untracked(), Some(selected_node));
            assert_eq!(synced_color.get_untracked(), Some(selected_color));
        });
    }
