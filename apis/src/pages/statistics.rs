...
use hive_lib::GameTypeFilter;
use leptos::prelude::*;
use leptos_meta::Style;
use shared_types::{GameSpeed, SiteStatisticsTimePeriod};
...
pub fn Statistics() -> impl IntoView {
    ...
    let selected_period = RwSignal::new(SiteStatisticsTimePeriod::default());
                                ...
                                match result {
                                    Ok(stats) => {
                                        ...
                                        let user_count = statistics_number_user_registrations
                                            .get()
                                            .unwrap_or(0);
                                        let games_by_type_and_time_control: Vec<TableRow> = {
                                            let speeds = GameSpeed::all_games();
                                            let stats_map: HashMap<GameSpeed, _> = stats
                                                .iter()
                                                .map(|s| (s.speed, s))
                                                .collect();
                                            speeds
                                                .iter()
                                                .map(|speed| {
                                                    if let Some(s) = stats_map.get(speed) {
                                                        TableRow {
                                                            label: TableRowLabel::Text(s.speed.to_string()),
                                                            values: vec![
                                                        ...
                                                        TableRow {
                                                            label: TableRowLabel::Text(speed.to_string()),
                                                            values: vec![0, 0, 0, 0],
                                                            additional_values: None,
                                                        }
                                                    }
                    ...
                    {move || {
                        statistics_most_active_players_by_period
                            ...
                            .map(|result| {
                                match result {
                                    Ok(stats) => {
                                        let speeds = GameSpeed::all_for_stats();
