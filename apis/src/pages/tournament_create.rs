    ...
    view! {
        <div class="flex justify-center items-center pt-10">
            <div class="container flex flex-col justify-between p-2 md:flex-row md:flex-wrap">
                <div class="basis-1/2"> <p class="text-3xl font-extrabold">Tournament settings: </p>
                ...
                <div>
                    ...
                    >
                        <SelectOption value=tournament.mode is="Round Robin"/>
                        <SelectOption value=tournament.mode is="Swiss"/>
                ...
                <input
                    ...
                    attr:max=move || (Local::now() + Duration::weeks(12)).format("%Y-%m-%dT%H:%M").to_string()
                    value=(Local::now() + Duration::days(1)).format("%Y-%m-%dT%H:%M").to_string()
                    on:input=move |evt| {
                        if let Ok(date) = NaiveDateTime::parse_from_str(&event_target_value(&evt), "%Y-%m-%dT%H:%M") {tournament.start_at.update(|v| *v = DateTime::<Utc>::from_naive_utc_and_offset(date, Utc))}
                        else {organizer_start.set(true)}
