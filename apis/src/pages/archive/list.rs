use crate::{components::molecules::game_row::GameRow, i18n::*, responses::GameResponse};
use leptos::prelude::*;
...
const PAGE_SIZE_SELECT_CLASS: &str =
    "select select-bordered select-sm w-20 pl-3 rounded-lg border-2 border-gray-300 dark:border-gray-600 bg-white text-gray-900 dark:bg-gray-800 dark:text-gray-100 h-11 min-h-11 font-medium focus:ring-2 focus:ring-pillbug-teal/50";
...
pub fn ArchiveGameList(
    ...
    // Brief "Copied!" confirmation, mirroring challenge_row's copy button.
    let copy_state = RwSignal::new(false);
    let copy_interval = StoredValue::new(use_interval_fn_with_options(
        move || copy_state.set(false),
        2000,
        UseIntervalFnOptions::default().immediate(false),
    ));
    let copy_permalink = move |_| {
        if let Some(window) = use_window().as_ref() {
            let _ = window.navigator().clipboard().write_text(&permalink_url());
        }
        copy_state.set(true);
        let interval = copy_interval.get_value();
        (interval.pause)();
        (interval.resume)();
    };
    let copy_button_class = move || {
        let base = "inline-flex gap-1.5 items-center px-3 h-11 text-sm font-medium rounded-lg border-2 shadow-sm transition-colors shrink-0";
        if copy_state.get() {
            format!("{base} text-white border-grasshopper-green bg-grasshopper-green")
        } else {
            format!("{base} text-gray-700 bg-white border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 hover:border-pillbug-teal/50 hover:bg-gray-100 dark:hover:bg-gray-700")
        }
    };
    ...
    view! {
        <div class="space-y-4">
            <Show when=move || total.get().is_some()>
                <div class="py-3 px-4 mx-auto max-w-5xl rounded-xl border border-gray-200 shadow-sm dark:border-gray-700 bg-white/60 dark:bg-gray-900/60">
                    <div class="flex flex-wrap gap-4 justify-between items-center">
                        <p class="text-sm font-medium text-gray-700 dark:text-gray-300 shrink-0">
                            {move || {
                                ...
                                if total_pg > 0 {
                                    ...
                                    t_string!(
                                        i18n, archive.page_info, page = p, total_pages = total_pg, start = start, end = end, total = t
                                    )
                                } else {
                                    t_string!(
                                        i18n, archive.games_loaded_count, loaded = 0, total = t
                                    )
                                }
                            }}
                        </p>
                        <div class="flex flex-wrap gap-3 items-center">
                            ...
                            <button
                                ...
                                aria-label=move || t_string!(i18n, archive.permalink).to_string()
            ...
            <div class="mx-auto space-y-4 w-full max-w-5xl">
                <Show when=move || has_searched.get()>
                    <div class="flex flex-col">
                        <div class="gap-3 min-h-0 rounded-xl sm:grid sm:grid-cols-2 sm:gap-4 sm:content-start lg:grid-cols-3">
                            <For each=move || games.get() key=|game| game.game_id.clone() let:game>
                                <GameRow game />
                            </For>
                        </div>
                        <Show when=move || games.with(|g| g.is_empty()) && !is_loading.get()>
                            <p class="py-8 mt-6 text-center text-gray-600 bg-gray-50 rounded-xl border border-gray-200 dark:text-gray-400 dark:border-gray-700 dark:bg-gray-900/50">
                                {t!(i18n, archive.no_games_found)}
                            </p>
                        </Show>
                        <Show when=is_loading>
                            <p class="py-8 mt-6 text-center text-gray-600 bg-gray-50 rounded-xl border border-gray-200 animate-pulse dark:text-gray-400 dark:border-gray-700 dark:bg-gray-900/50">
