...
pub fn Tournament() -> impl IntoView {
    ...
    let current_tournament = Signal::derive(move || {
    ...
    view! {
        <div class="flex flex-col justify-center items-center pt-20 w-full">
            <Show when=move || {
                current_tournament().is_some()
            }>
                <LoadedTournament tournament=Signal::derive(move || current_tournament().expect("It is some"))/>
            </Show>
        </div>
    }
}
...
#[component]
pub fn LoadedTournament(tournament: Signal<TournamentResponse>) -> impl IntoView {
    ...
    let user_uninvite = move || {
        if user_is_organizer() {
            vec![UserAction::Uninvite(tournament_id())]
        } else {
            vec![]
        }
    };
            ...
            <div class="flex flex-col flex-wrap place-content-center md:flex-row">
                ...
                >
                    ...
                    <div class="flex flex-col items-center w-full">
                        ...
                        <For
                            each=move || {
                                tournament().standings.results().into_iter()
                            }
                        ...
                        <div class="flex flex-wrap justify-center items-center">
                            <GamePreviews games=Callback::new(move |_| {
                                tournament().games
                            })/>
                        </div>
                    </div>
