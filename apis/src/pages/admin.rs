use crate::{components::{atoms::simple_switch::SimpleSwitch, molecules::rl_banner::RlBanner, organisms::chat::ChatWindow, update_from_event::update_from_input}, functions::home_banner, providers::AuthContext};
use leptos::prelude::*;
...
fn EditBanner() -> impl IntoView {
    let banner = OnceResource::new( async move {
        home_banner::get_with_display().await.unwrap_or_default()
    });
    view! {
        <Suspense>
        {move || banner.get().map(|(banner, display)| {
            ...
            >
                {move || if !show_preview() { "Preview Banner" } else { "Edit Banner" }}
                </button>
                ...
                <div class=move || if show_preview() { "" } else { "hidden" }>
                {
                    move || {
                        view! {
                            <RlBanner title=title() content=content() />
                        }
                    }
                }
                </div>
                <div class=move || if !show_preview() { "" } else { "hidden" }>
                    ...
                    <div class="m-4">
                        ...
                        {move || if display() { "Display banner" } else { "Hide banner" }}
                        <SimpleSwitch checked=display />
                        <input class="hidden" type="text" name="display" prop:value=display />
                    </div>
                    </ActionForm>
            }
        })}
