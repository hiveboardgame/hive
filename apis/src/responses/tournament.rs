...
#[cfg(feature = "ssr")]
mod ssr {
    use super::TournamentResponse;
    use crate::responses::{game::GameResponseDb, user::UserResponseDb};
    use anyhow::Result;
    ...
    pub trait TournamentAbstractResponseDb: Sized {
        fn from_uuid(
            id: &Uuid,
            conn: &mut DbConn<'_>,
    ...
    impl TournamentAbstractResponseDb for TournamentAbstractResponse {
        ...
        async fn from_model(tournament: &Tournament, conn: &mut DbConn<'_>) -> Result<Self> {
            let player_list = tournament
                .players(conn)
                .await?
                .iter()
                .map(|p| p.id)
        ...
        pub async fn from_model(
            ...
            Ok(Box::new(Self {
                ...
                tiebreakers: tournament
                    ...
                    .into_iter()
                    .flatten()
                    .flat_map(|t| Tiebreaker::from_str(&t).ok())
                    .collect(),
                invitees,
                seats: tournament.seats,
...
#[cfg(feature = "ssr")]
pub use ssr::TournamentAbstractResponseDb;
