...
impl Timer {
    ...
    pub fn low_time_trigger_at(&self) -> Option<Duration> {
        if self.time_mode == TimeMode::RealTime {
            self.time_base
                .map(|base| base / 10 + self.time_increment.unwrap_or(Duration::from_secs(0)) * 2)
        } else {
            None
        }
    }
