...
pub fn BaseLayout(children: ChildrenFn) -> impl IntoView {
    ...
    create_effect(move |_| {
        ...
        let game_nanoid = if let Some(caps) = GAME_NANOID.captures(&pathname) {
            caps.name("nanoid").map(|m| m.as_str().to_string())
        } else {
            None
        };
        let torunament_nanoid = if let Some(caps) = TOURNAMENT_NANOID.captures(&pathname) {
            caps.name("nanoid").map(|m| m.as_str().to_string())
        } else {
            None
        };
        if ws_ready() == ConnectionReadyState::Open {
            navi.update_nanoids(game_nanoid, torunament_nanoid);
        };
    });
