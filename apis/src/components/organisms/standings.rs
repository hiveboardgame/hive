    ...
    view! {
        <th class=TH_CLASS>
            <div node_ref=container_ref class="relative">
                ...
                <Show when=is_open>
                    <DropdownPanel class="absolute left-1/2 top-full z-50 p-2 mt-1 text-xs font-normal text-left text-gray-900 normal-case -translate-x-1/2 dark:text-gray-200 w-fit max-w-[18rem] text-wrap">
...
pub fn Standings(
    ...
    #[prop(optional, into)] max_height: Option<Signal<Option<f64>>>,
) -> impl IntoView {
    let tiebreakers = tournament.with_untracked(|t| t.tiebreakers.clone());
    let panel_style = max_height.map(|max_height| {
        move || max_height().map(|height| format!("--tournament-info-height: {height}px"))
    });
    ...
    view! {
        <div class="min-w-0 lg:h-[var(--tournament-info-height)]" style=panel_style>
            <Panel
                title="Standings"
                class="min-w-0 lg:flex lg:overflow-hidden lg:flex-col lg:h-full"
                body_class="min-h-0 overflow-auto"
            >
