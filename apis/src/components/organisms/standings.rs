...
fn TiebreakerHeader(tiebreaker: Tiebreaker, open_tooltip: RwSignal<Option<Tiebreaker>>) -> impl IntoView {
    ...
    let explanation = match &tiebreaker {
        ...
        Tiebreaker::SonnebornBerger => t_string!(i18n, tournaments.tiebreakers.sonneborn_berger),
                ...
                <th class=TH_CLASS>
                    ...
                    >
                        <span class="hover:cursor-help" title=explanation>
                ...
                <Show when=is_open>
                    <div 
                        class="absolute top-full left-1/2 transform -translate-x-1/2 mt-1 z-50 w-64 p-2 text-xs font-normal normal-case text-left bg-gray-900 text-white rounded-lg shadow-lg dark:bg-gray-700 dark:text-gray-200"
...
pub fn Standings(tournament: Signal<TournamentResponse>) -> impl IntoView {
    ...
    let close_tooltip = move |_: ev::MouseEvent| {
        open_tooltip.set(None);
    };
    ...
    view! {
        <div class="relative" on:click=close_tooltip>
