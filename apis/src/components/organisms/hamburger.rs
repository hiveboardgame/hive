    ...
    view! {
        <div node_ref=target class="inline-block mr-10">
            <button
                ...
                class="bg-blue-500 text-white rounded-md px-2 py-1 m-2 hover:bg-blue-600 transform transition-transform duration-300 active:scale-95 whitespace-nowrap"
            >
                {username}
            </button>
            <Show when=hamburger_show>
                <div class="flex flex-col items-stretch absolute bg-slate-400 text-black border border-gray-300 rounded-md right-[50px] p-2">
