...
pub fn GamesFilter(username: String, ctx: GamesSearchContext) -> impl IntoView {
    ...
    let apply_filters = move |_| {
        perform_search(pending.get());
        close_dropdown();
    };

    let reset_filters = move |_| {
        let saved_defaults = ctx.get_filter_cookie.get().unwrap_or_default();
        pending.set(saved_defaults.clone());

        if ctx.filters.get() != saved_defaults {
            perform_search(saved_defaults);
        }
        close_dropdown();
    };
    ...
    let set_default_filters = move |_| {
        ...
        if ctx.filters.get() != current_pending {
    ...
    let clear_default_filters = move |_| {
        ...
        if ctx.filters.get() != system_defaults {
    ...
    let reset_disabled = Signal::derive(move || {
        ctx.filters.get() == ctx.get_filter_cookie.get().unwrap_or_default()
    });
    ...
    view! {
        <div class="relative">
            <details node_ref=dropdown_ref>
                ...
                <div class="absolute right-0 top-full z-40 p-2 mt-1 max-w-screen-sm bg-white rounded-lg border border-gray-200 shadow-lg w-[90vw] lg:p-0 lg:mt-2 lg:shadow-xl dark:bg-gray-900 dark:border-gray-700">
                    ...
                    <div class="space-y-2 lg:space-y-0 lg:flex">
                        <div class="space-y-2 lg:flex-1 lg:p-4 lg:border-r lg:border-gray-200 lg:dark:border-gray-700">
                            ...
                            <Show when=move || current_tab() == GameProgress::Finished>
                                <div class="space-y-1 lg:space-y-2">
                                    ...
                                    <div class="grid grid-cols-2 gap-1 lg:gap-2">
                                        <FilterButton
                                            is_active=Signal::derive(move || pending.with(|p| p.result == Some(ResultType::Win)))
                                            on_click=move |_| pending.update(|state| state.result = Some(ResultType::Win))
                                        >
                                            {t!(i18n, profile.result_buttons.win)}
                                        </FilterButton>
                                        <FilterButton
                                            is_active=Signal::derive(move || pending.with(|p| p.result == Some(ResultType::Loss)))
                                            on_click=move |_| pending.update(|state| state.result = Some(ResultType::Loss))
                                        >
                                            {t!(i18n, profile.result_buttons.loss)}
                                        </FilterButton>
                                        <FilterButton
                                            is_active=Signal::derive(move || pending.with(|p| p.result == Some(ResultType::Draw)))
                                            on_click=move |_| pending.update(|state| state.result = Some(ResultType::Draw))
                                        >
                                            {t!(i18n, profile.result_buttons.draw)}
                                        </FilterButton>
                                        <FilterButton
                                            is_active=Signal::derive(move || pending.with(|p| p.result.is_none()))
                                            on_click=move |_| pending.update(|state| state.result = None)
                                        >
                                            "Any"
                                        </FilterButton>
                                    </div>
                            ...
                            <div class="space-y-1 lg:space-y-2">
                                ...
                                <div class="grid grid-cols-3 gap-1 lg:gap-2">
                                    {GameSpeed::all_games().into_iter().map(|speed| view! {
                                        <FilterButton
                                            is_active=Signal::derive(move || pending.with(|state| state.speeds.contains(&speed)))
                        ...
                        <div class="space-y-2 lg:flex-1 lg:p-4">
                            ...
                            <div class="flex justify-between items-center lg:pt-2">
                                <span class="text-xs font-medium text-gray-700 dark:text-gray-300 lg:text-sm">"Bot Games:"</span>
                                <FilterButton
                                    is_active=Signal::derive(move || !pending.with(|state| state.exclude_bots))
                                    on_click=move |_| pending.update(|state| state.exclude_bots = !state.exclude_bots)
                                    flex_class=""
                                >
                                    {move || if pending.with(|state| state.exclude_bots) { "Exclude" } else { "Include" }}
                                </FilterButton>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-1 pt-2 mt-3 border-t border-gray-200 xs:gap-2 dark:border-gray-700 lg:gap-3 lg:justify-end lg:p-4 lg:bg-gray-50 lg:dark:bg-gray-800/30 lg:border-gray-200 lg:dark:border-gray-700">
                        ...
                        <ActionButton
                            ...
                            disabled=Signal::derive(move || is_current_saved_default() || no_speeds())
                        ...
                        <ActionButton
                            ...
                            disabled=Signal::derive(move || !saved_default_exists() && is_system_default())
    ...
    view! {
        <div class="pb-2 mb-2 border-b border-gray-200 dark:border-gray-700">
            <div class="flex flex-wrap gap-1 items-center min-h-[24px]">
                ...
                <Show when=move || {
                    current_tab() == GameProgress::Finished && ctx.filters.with(|state| state.result.is_some())
                }>
                    <FilterPill>
    ...
    view! {
        <div class="space-y-1 lg:space-y-2">
            ...
            <div class=format!("grid grid-cols-3 gap-1 {}", grid_class)>
                <FilterButton
                    is_active=is_option1_active
                    on_click=set_option1
                    flex_class=""
                >
                    {move || match filter_type {
                ...
                <FilterButton
                    is_active=is_option2_active
                    on_click=set_option2
                    flex_class=""
                >
                    {move || match filter_type {
