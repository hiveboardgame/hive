...
use chrono::{Duration, Datelike, DateTime, Local, TimeZone, Utc};

fn bucket_by_day(data: Vec<RatingHistoryResponse>) -> Vec<RatingHistoryResponse> {
    data.into_iter()
        .sorted_by_key(|r| r.updated_at)
        .chunk_by(|r| {
            let local = r.updated_at.with_timezone(&Local);
            (local.year(), local.month(), local.day())
        })
        .into_iter()
        .filter_map(|((_y, _m, _d), group)| {
            let last = group.last()?;
            // Convert back to UTC for the chart
            let local_day = last.updated_at.with_timezone(&Local).date_naive();
            let utc_day = Utc.from_local_datetime(&local_day.and_hms_opt(0, 0, 0).unwrap()).unwrap();
            Some(RatingHistoryResponse {
                rating: last.rating,
                updated_at: utc_day,
            })
        })
...
fn build_x_ticks(data: ReadSignal<Vec<RatingHistoryResponse>>) -> TickLabels<DateTime<Utc>> {
    ...
    let min_time = data_vec.iter().map(|r| r.updated_at).min().unwrap();
    let max_time = data_vec.iter().map(|r| r.updated_at).max().unwrap();
...
pub fn RatingGraph(user_id: Uuid, game_speed: GameSpeed) -> impl IntoView {
    ...
    let history_local = Signal::derive(move || {
        history.get().map(|res| {
                match res {
                    Ok(hist) => Ok(bucket_by_day(hist.clone())),
                    Err(e) => Err(e.clone()),
                }
            })
    });
