...
pub fn UserWithRating(
    ...
    let player = Memo::new(move |_| match side() {
        Color::White => {
            game_response.with(|game| game.as_ref().map(|game| game.white_player.clone()))
        }
        Color::Black => {
            game_response.with(|game| game.as_ref().map(|game| game.black_player.clone()))
        }
    });
    let speed = Memo::new(move |_| {
        game_response.with(|game| {
            game.as_ref().map(|resp| match resp.speed {
